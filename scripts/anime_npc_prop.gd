extends Node3D
class_name AnimeNpcProp
## Reusable, stationary wrapper for the supplied anime-style PSX character.

const CHARACTER_SCENE: PackedScene = preload(
	"res://assets/psx character npc/anime-male-character-psx-style/source/suluh_character.glb"
)

const COLLISION_CENTER := Vector3(0.0, 0.86, 0.0)
const COLLISION_SIZE := Vector3(0.46, 1.70, 0.40)
...
func build() -> void:
	...
	if _visual_model == null:
		push_error("AnimeNpcProp could not instantiate suluh_character.glb")
		return
	_visual_model.name = "CharacterModel"
	# Match the project's local -Z character-forward convention.
...
func _play_idle_animation() -> void:
	...
	if _animation_player == null:
		push_warning("AnimeNpcProp: AnimationPlayer not found")
		return
	var animation_name := _choose_animation(_animation_player, ["idle"])
	if animation_name == &"":
		push_warning("AnimeNpcProp: no animation clip found")
		return
